 ...
 * a ModifyTable node referencing a pgc_fdw foreign table.  We store:
 *
...
/*
 * Execution state of a foreign scan using pgc_fdw.
 */
 ...
 * SQL functions
 */
PG_FUNCTION_INFO_V1(pgc_fdw_handler);
 ...
 */
Datum
pgc_fdw_handler(PG_FUNCTION_ARGS)
{
								...
								Relation target_relation)
{
	...
	/*
	 * In pgc_fdw, what we need is the ctid, same as for a regular table.
	 */
...
postgresIsForeignRelUpdatable(Relation rel)
{
	...
	/*
	 * By default, all pgc_fdw foreign tables are assumed updatable. This
					 ...
					 const char ***param_values)
{
	...
	*param_flinfo = (FmgrInfo *) palloc0(sizeof(FmgrInfo) * numParams);
	 ...
	 * benefit, and it'd require pgc_fdw to know more than is desirable
...
/*
 * Acquire a random sample of rows from foreign table managed by pgc_fdw.
 *
...
void cache_create_cursor(ForeignScanState *node)
{
	...
	/*
	 * XXX: 
	 * We should have put in a uniqifier for each foreign server.
	 */
